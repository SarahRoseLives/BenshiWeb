// lib/benshi/protocol/data_models.dart
...
class Channel {
  ...
  factory Channel.fromJson(Map<String, dynamic> json) {
    T _parseEnum<T extends Enum>(List<T> values, dynamic value, T defaultValue) {
      ...
      if (value is String) {
        return values.firstWhere((e) => e.name.toLowerCase() == value.toLowerCase(), orElse: () => defaultValue);
      }
      return defaultValue;
    }
  ...
  String get txTone => formatSubAudio(txSubAudio);
  String get rxTone => formatSubAudio(rxSubAudio);

  static String formatSubAudio(dynamic val) {
  ...
  static dynamic parseSubAudioFromString(String val) {
    if (val.trim().isEmpty || val.toLowerCase() == 'none') {
      return null;
    }
    final lowerVal = val.toLowerCase();
    if (lowerVal.startsWith('dcs')) {
      return int.tryParse(lowerVal.replaceAll('dcs', '').trim());
    }
    return double.tryParse(lowerVal.replaceAll('hz', '').trim());
  }
}
...
factory Settings.fromBytes(Uint8List bytes) {
  ...
  Uint8List toBytes() {
    final w = ByteWriter(20);
    w.writeInt(channelA & 0x0F, 4);
    w.writeInt(channelB & 0x0F, 4);
    w.writeBool(scan);
...
class TncDataFragment {
  ...
  factory TncDataFragment.fromBytes(Uint8List bytes) {
    final r = ByteReader(bytes);
    final isFinalFragment = r.readBool();
    final withChannelId = r.readBool();
    final fragmentId = r.readInt(6);
